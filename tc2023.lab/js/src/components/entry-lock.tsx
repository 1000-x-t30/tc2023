...
const dispatchEntryExclusiveControl = () => {
  if ((ACMS.Config.admin === 'entry-edit' || ACMS.Config.admin === 'entry_editor')) {
    const entryEditForm = document.querySelector('#entryForm');
    if (entryEditForm) {
      // ToDo: カテゴリー選択やメディアの画像選択などのイベントもフックしたい
      entryEditForm.addEventListener('input', throttle(10000, async () => {
            ...
            <tr>
              ...
              <td>
                <div className="acms-admin-entry-lock-modal-user">
                  <img src={`${ACMS.Config.root}archives/${lockedInfo.icon}`} width="24" height="24" className="acms-admin-user" alt={lockedInfo.name} />
